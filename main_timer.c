...
static const rom char CopyrString[]= {'T','i','m','e','r',' ','W','1','2','0','9',' ',
#ifdef USA_USB
'c','o','n',' ','U','S','B',' ',
#endif
...
resetta:
		...
		configParms.currentDate.mday=23;
	...
	if(!HIDRxHandleBusy(USBOutHandle)) {   
	  switch(ReceivedDataBuffer[1]) {			// ReceivedDataBuffer[0] contiene 2 = OUTPUT_REPORT, se ESPLICITAMENTE DEFINITO
			...
			case CMD_GETCONFIG:											// restituisce config. 
				...
				ToSendDataBuffer[6]=LOBYTE(configParms.timerCount);
				ToSendDataBuffer[7]=HIBYTE(configParms.timerCount);
				ToSendDataBuffer[8]=Buzzer;

				ToSendDataBuffer[26]=configParms.timerUnit;
				ToSendDataBuffer[27]=configParms.timerMode;
				ToSendDataBuffer[28]=configParms.timerBoot;
...
//				ToSendDataBuffer[6]=batteryState;
				...
				ToSendDataBuffer[24]=LOBYTE(configParms.timerCount);
				ToSendDataBuffer[25]=HIBYTE(configParms.timerCount);
